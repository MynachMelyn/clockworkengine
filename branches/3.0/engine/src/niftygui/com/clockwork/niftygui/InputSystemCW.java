...
public class InputSystemCW implements InputSystem, RawInputListener {
    ...
    public InputSystemCW(InputManager inputManager) {
    ...
    private void handleMouseEvent(int button, boolean value, NiftyInputConsumer nic, InputEvent evt) {
        if (value) {
            ...
            // b) when the CW mouse cursor is visible.
            //
            // That's ok but the "Nifty remembers the mouse down event" thing had the following consequences in one
            // special case:
            // 1) You click on the CW scene (not Nifty) and Nifty will correctly return false (event not consumed) but
            //    internally it remembers: "mouse button is now down". Note that the CW mouse cursor is now hidden.
            // 2) You release the mouse button but the mouse down event will not be forwarded to Nifty because it did
            //    owned the mouse and the CW mouse cursor is not visible.
            //
            ...
            // or the CW mouse cursor visibility.
            //
            // Please note: Compared to the original version a side effect is that CW will now always send mouse move
            // events to Nifty even when the mouse cursor is hidden. So in theory it could happen that input events are
            // handled by both: CW and Nifty when f.i. you move around your scene with the mouse cursor hidden and that
...
//        }
    ...
    private void processSoftKeyboard() {
        SoftTextDialogInput softTextDialogInput = CWSystem.getSoftTextDialogInput();
        if (softTextDialogInput != null) {
            ...
            if (element != null) {
                ...
                if (textField != null && !(textField instanceof TextFieldNull)) {
                    Logger.getLogger(InputSystemCW.class.getName()).log(Level.FINE, "Current TextField: {0}", textField.getId());
