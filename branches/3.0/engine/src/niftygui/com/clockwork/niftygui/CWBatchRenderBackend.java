 ...
 * 
 */
public class CWBatchRenderBackend implements BatchRenderBackend {
  private static Logger log = Logger.getLogger(CWBatchRenderBackend.class.getName());

  private final ObjectPool<Batch> batchPool;
  private final List<Batch> batches = new ArrayList<Batch>();

  // a modify texture call needs a CW Renderer to execute. if we're called to modify a texture but don't
  // have a Renderer yet - since it was not initialized on the CW side - we'll cache the modify texture calls
  ...
  private boolean fillRemovedTexture =
      Boolean.getBoolean(System.getProperty(CWBatchRenderBackend.class.getName() + ".fillRemovedTexture", "false"));

  public CWBatchRenderBackend(final NiftyCWDisplay display) {
  ...
  private static class ImageImpl implements BatchRenderBackend.Image {
    ...
    public void modifyTexture(
        final CWBatchRenderBackend backend,
        final Texture2D textureAtlas,
        final int x,
        final int y) {
  ...
  /**
   * Used to delay ModifyTexture calls in case we don't have a CW Renderer yet.
   * 
   */
