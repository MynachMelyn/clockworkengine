...
public class RenderDeviceCW implements RenderDevice {
...
//            hash = 53 * hash + color.hashCode();
    ...
    public RenderDeviceCW(NiftyCWDisplay display) {
    ...
    public RenderImage createImage(String filename, boolean linear) {
        ...
        return new RenderImageCW(filename, linear, display);
    }
    
    public RenderFont createFont(String filename) {
        return new RenderFontCW(filename, display);
    }
    ...
    public void renderFont(RenderFont font, String str, int x, int y, Color color, float sizeX, float sizeY) {        
        ...
        RenderFontCW CWFont = (RenderFontCW) font;
        
        ColorRGBA colorRgba = convertColor(color, tempColor);
        CachedTextKey key = new CachedTextKey(CWFont.getFont(), str/*, colorRgba*/);
        BitmapText text = textCacheLastFrame.get(key);
        if (text == null) {
            text = CWFont.createText();
            text.setText(str);
...
//        System.out.format("renderFont(%s, %s, %d, %d, %s, %f, %f)\n", CWFont.getFont(), str, x, y, color.toString(), sizeX, sizeY);
    }
    ...
    public void renderImage(RenderImage image, int x, int y, int w, int h,
        ...
        RenderImageCW CWImage = (RenderImageCW) image;
        Texture2D texture = CWImage.getTexture();
        
        textureColorMaterial.setColor("Color", convertColor(color, tempColor));
        textureColorMaterial.setTexture("ColorMap", texture);        
        
        float imageWidth = CWImage.getWidth();
        float imageHeight = CWImage.getHeight();
    ...
    public void renderImage(RenderImage image, int x, int y, int width, int height,
        ...
        textureColorMaterial.setColor("Color", convertColor(color, tempColor));
        textureColorMaterial.setTexture("ColorMap", CWImage.getTexture());
        ...
        //System.out.format("renderImage1(%s, %d, %d, %d, %d, %s, %f)\n", CWImage.getTexture().getKey().toString(), x, y, width, height, color.toString(), imageScale);
    }
