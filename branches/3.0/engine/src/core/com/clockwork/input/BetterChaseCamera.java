...
public class BetterChaseCamera implements ActionListener, AnalogListener, Control/*, JMECloneable */{
     ...
     */
    public BetterChaseCamera(Camera cam, final Spatial target) {
        this(cam);
     ...
     * @param cam the application camera
     */
    public BetterChaseCamera(Camera cam) {
        this.cam = cam;
     ...
     */
    public BetterChaseCamera(Camera cam, InputManager inputManager) {
        this(cam);
     ...
     * @param inputManager the inputManager of the application to register inputs
     */
    public BetterChaseCamera(Camera cam, final Spatial target, InputManager inputManager) {
        this(cam, target);
        registerWithInput(inputManager);
    }
    ...
    */
    ...
    public void forceLookAt(Vector3f location){
        cam.setLocation(target.getLocalTranslation().add(new Vector3f(0, 10, 0)));
        cam.lookAt(location, initialUpVec);
    }
    ...
    /**
     * Force the rotation, as in without mouse input.
     */
    /*public void setForcedRotation(float horizontalRot){
        if(!canRotate){
            canRotate = true;
